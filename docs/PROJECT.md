...
| [systems/ui.md](systems/ui.md) | HUD, TowerPanel, 머지 프리뷰(조합 목록+드래그 하이라이트+호버 말풍선), 일시정지(합성도감 버튼 포함), 게임속도, 골드 싱크 UI, 컬렉션(이중 탭: 메타업그레이드+합성도감), MergeCodexScene(합성도감 전용 씬, 노드 트리 오버레이, T1 메타 업그레이드 스탯, T2+ 스탯, 상위 조합 리스트), 모바일 |
