...
각 타워는 i18n.js에 다음 키를 보유한다:

| 키 패턴 | 용도 | 적용 범위 |
|---|---|---|
| `tower.{id}.name` | 타워 이름 | T1~T5 전체 |
| `tower.{id}.flavor` | 분위기 텍스트 (이탤릭) | T1 10종 |
| `tower.{id}.desc` | 특징 설명 (공격 타입 + 핵심 특성) | T1~T5 전체 |

### 표시 위치

- **TowerPanel 롱프레스 팝업**: flavor + desc 2줄 (T1)
- **TowerInfoOverlay T1 패널**: flavor + desc 2줄 (이름 아래, 공격 유형 배지 위)
- **TowerInfoOverlay T2+ 트리 패널**: desc 1줄 (결과 노드 이름 아래)
- **CollectionScene 상세뷰**: flavor + desc 2줄 (이름/메타티어 아래, 구분선 위)

## 타워 획득: 랜덤 뽑기 시스템

타워 배치는 개별 타워 직접 구매가 아닌 **단일 뽑기 버튼**을 통해 이루어진다.

- 뽑기 클릭 시 해금된 T1 타워 중 균등 확률로 1종을 랜덤 선택
- 선택된 타워로 기존 배치 모드(맵 탭 > 셀 선택) 진입
- 비용 공식: `DRAW_BASE_COST + DRAW_COST_INCREMENT * drawCount` = **80 + 20 * drawCount**
- 비용 예시: 0회차 80G, 1회차 100G, 5회차 180G, 10회차 280G
- **배치 확정 시**에만 골드 차감 및 횟수 증가. 배치 취소 시 무효
- `drawCount`는 게임 세션 단위 (저장 불필요, 재시작 시 0)
- 뽑기 풀: 기본 5종(archer, mage, ice, lightning, flame) + 해금된 타워
- 롱프레스(400ms) 시 현재 뽑기 풀 목록/확률 팝업 표시

> 기존 `TOWER_STATS[type].levels[1].cost` (개별 설치비)는 판매 환급 계산 등에 여전히 사용된다.

## 타워 목록

| 타워 | 기본 비용 | 공격 타입 | 핵심 특성 | 잠금 |
|---|---|---|---|---|
...
- 합성 비용: 티어별 골드 비용 (`MERGE_COST` -- T2: 30G, T3: 80G, T4: 150G, T5: 250G)
  - 골드 부족 시 합성 거부 + 경고 메시지
- 합성 결과 위치: 드롭 대상(B) 위치에 생성
- 판매 가격: `floor(totalInvested * SELL_RATIO)` -- totalInvested는 두 재료의 투자 비용 + 합성 비용 합산
...
| 공격 타입 | 수량 | 타워 ID |
|---|---|---|
| chain | 7 | thunder_wyrm_king, adamantine_lightning, superconductor_mage, lightning_tempest, storm_deity, thunderstorm_king, sacred_judgement_beam |
| aoe_instant | 12 | plague_wyrm, cosmos_dragon, venom_sovereign, absolute_frost_domain, grand_pyromancer, molten_hell, eternal_blizzard, great_poison_king, death_miasma, solar_cannon, great_typhoon, permafrost |
| splash | 9 | tectonic_dragon, hellquake, primal_dragon_lord, iron_dragon, wind_dragon_king, arcane_dragon_sage, sky_lancer, earth_shatterer, arcane_cannon |
...
| 조합 키 | ID | 이름 | 공격 타입 | 주요 스탯 |
|---|---|---|---|---|
| genesis_verdict+void_sniper | omega_herald | 오메가 전령 | chain | dmg:350, spd:0.8, rng:600, chain:16, decay:0.92, burn:25/7s, poison:18/8s, armorPiercing |
| pandemic_sovereign+world_breaker | extinction_engine | 소멸 기관 | aoe_instant | dmg:120, spd:1.5, rng:280, splash:220, poison:18/8s, stacks:8, armor:-40%, burn:20/6s |
| glacial_epoch+storm_dominion | absolute_dominion | 절대 지배 | chain | dmg:250, spd:0.8, rng:260, chain:14, decay:0.88, slow:0.9/5.0s, push:180/15 |
| magma_core+solar_cremation | star_forge | 항성 용광로 | aoe_instant | dmg:120, spd:0.8, rng:260, splash:220, burn:50/8s |
| annihilation_gale+maelstrom_herald | void_maelstrom | 공허 소용돌이 | piercing_beam | dmg:150, spd:1.2, rng:600, burn:30/7s, poison:25/8s, push:300, armorPiercing |
...
배치된 타워를 드래그하여 빈 셀로 이동할 수 있다. 기존 합성 드래그 흐름에 이동 분기를 통합하여 하나의 드래그 동작으로 이동/합성/취소를 처리한다.

### 동작 규칙

| 드롭 대상 | 동작 |
|---|---|
| 빈 배치 가능 셀 (CELL_EMPTY, 타워 없음) | 해당 셀로 이동 (무료) |
| 합성 가능 타워 (enhanceLevel === 0, 레시피 존재) | 기존 합성 실행 |
| 합성 불가 타워 (enhanceLevel > 0 또는 레시피 없음) | 흔들기 피드백 후 원위치 복원 |
| 배치 불가 셀 (경로/벽/기지/스폰) | 원위치 복원 |
| 맵 밖 / 범위 밖 좌표 | 원위치 복원 |

### 이동 처리 흐름

1. `TowerPanel.startDrag()` -- 드래그 시작 시 `onDragStart` 콜백으로 빈 셀 초록 하이라이트 표시 (`MapManager.showBuildableHighlights()`)
2. `TowerPanel.endDrag()` -- 드롭 위치에 타워가 없고 `isBuildable(col, row)` === true이면 `onMove(tower, col, row)` 콜백 호출
3. `GameScene._onTowerMove()` -- `MapManager.moveTower()`로 내부 towerMap 갱신 -> 타워의 col/row/x/y 좌표 갱신 -> sprite 위치 갱신 -> `draw()` 재렌더링
4. `TowerPanel.endDrag()` -- 드래그 종료 시 `onDragEnd` 콜백으로 하이라이트 제거 (`MapManager.clearHighlights()`)

### 제약사항

- 이동 비용 무료 (골드 차감 없음)
- 강화된 타워(enhanceLevel > 0)는 드래그 자체 불가 (기존 합성 제약과 동일)
- 이동 후 원래 셀은 빈 셀로 처리되어 새 타워 배치 가능
- `MapManager.moveTower()`는 `towerMap.delete()` + `towerMap.set()`만 수행, 그래픽 좌표 갱신은 GameScene에서 처리

### 관련 파일

| 파일 | 메서드/변경 |
|---|---|
| `js/managers/MapManager.js` | `moveTower(fromCol, fromRow, toCol, toRow, tower)` |
| `js/ui/TowerPanel.js` | `startDrag()` onDragStart 콜백, `endDrag()` isBuildable 파라미터 + 빈 셀 이동 분기 + onDragEnd 콜백 |
| `js/scenes/GameScene.js` | `_onTowerMove(tower, col, row)`, onMove/onDragStart/onDragEnd 콜백 연결, endDrag 호출 시 isBuildable 전달 |
