...
**AdMob 통합 완료** (2026-03-02), 보상형 광고 타워 뽑기 추가 (2026-03-04)
...
- 타워 뽑기: 완료 (보상형 광고: 타워 3개 선택 무료 배치, 일일 제한 없음)
...
### isBusy 플래그 (Race Condition 방지)

`showRewarded()` / `showInterstitial()`은 async 함수로 광고 로딩-표시-완료까지 수 초가 소요될 수 있다. 이 대기 시간 동안 씬 전환 버튼이 인터랙티브 상태이면 파괴된 씬의 게임 오브젝트에 접근하는 오류가 발생할 수 있다.

- **프로퍼티**: `isBusy: boolean` (퍼블릭, 기본값 `false`)
- **설정 시점**: `showRewarded()` / `showInterstitial()` 진입 즉시 `true`
- **해제 시점**: Mock 경로 `return` 직전 `false`, 실제 광고 경로 `finally` 블록에서 `false` (성공/실패/예외 무관 보장)
- **가드 패턴**: 네비게이션 버튼 핸들러 최상단에서 `const adManager = this.registry.get('adManager'); if (adManager?.isBusy) return;`
- **옵셔널 체이닝**: adManager가 null/undefined이면 `isBusy`는 `undefined`(falsy)로 평가되어 버튼 정상 동작

#### 가드 적용 씬/버튼 (14개)

| 씬 | 버튼 | 행 |
|---|---|---|
| LevelSelectScene | BACK | 80-81 |
| LevelSelectScene | START | 212-213 |
| MenuScene | CAMPAIGN | 124-125 |
| MenuScene | ENDLESS | 151-152 |
| MenuScene | COLLECTION | 195-196 |
| MenuScene | STATISTICS | 218-219 |
| GameOverScene | RETRY | 267-268 |
| GameOverScene | WORLD MAP | 294-295 |
| GameOverScene | MENU (캠페인) | 317-318 |
| GameOverScene | MENU (엔드리스) | 340-341 |
| MapClearScene | NEXT MAP | 352-353 |
| MapClearScene | RETRY | 384-385 |
| MapClearScene | WORLD MAP | 411-412 |

MapClearScene의 3개 버튼은 `_ensureSaved()` 호출 이전에 가드가 배치되어, 광고 진행 중 세이브 처리도 방지한다.

#### 씬 활성 상태 체크 (isActive, 5개)

광고 콜백(await 이후)에서 씬이 이미 전환된 경우 파괴된 게임 오브젝트 접근을 방지한다.

| 씬 | 메서드 | 행 |
|---|---|---|
| LevelSelectScene | `_createGoldBoostButton` | 332 |
| LevelSelectScene | `_createClearBoostButton` | 420 |
| MenuScene | `_createDiamondAdButton` | 339 |
| GameOverScene | `_createReviveButton` | 402 |
| MapClearScene | `_createClearBoostButton` | 461 |

#### 비적용 대상

- 음소거 토글 버튼 (씬 전환 없이 상태만 변경)
- 종료 다이얼로그 (앱 종료 동작, 별도 `__isExitDialogOpen` 가드 보유)
- 광고 버튼이 없는 씬(EndlessMapSelectScene, WorldSelectScene, CollectionScene, StatsScene)
...
| `draw.ad.button` | 무료뽑기 | Free Draw | 타워 뽑기 |
| `draw.ad.sublabel` | 타워 3선택 | Pick 1 of 3 | 타워 뽑기 |
| `draw.ad.modal.title` | 광고 보상: 타워 선택 | Ad Reward: Pick a Tower | 타워 뽑기 |
| `draw.ad.modal.select` | 선택 | Select | 타워 뽑기 |
| `draw.ad.modal.cancel` | 취소 | Cancel | 타워 뽑기 |
| `draw.ad.failed` | 광고를 불러올 수 없습니다 | Ad unavailable | 타워 뽑기 |
| `draw.ad.badge.t1` | T1 | T1 | 타워 뽑기 |
| `draw.ad.badge.t2` | T2 ★ | T2 ★ | 타워 뽑기 |
...
## 보상형 광고 5: 타워 뽑기 -- 2026-03-04

TowerPanel에서 보상형 광고를 시청하고 T1(90%)/T2(10%) 타워 3개 중 1개를 무료로 배치한다.

### 버튼 배치

- **위치**: TowerPanel 1행 우측 (X=265, Y=PANEL_Y+42), 뽑기 버튼(X=95) 오른쪽
- **크기**: 150x44px (뽑기 버튼과 동일)
- **배경**: `slot_action_normal` 텍스처, 폴백 사각형(0x1a1a2e, BTN_META 테두리)
- **텍스트**: 상단 "무료뽑기" (i18n: `draw.ad.button`, adFree 여부 무관 동일 키), 하단 "타워 3선택 (N/3)" (i18n: `draw.ad.sublabel`, 10px 연보라 #9b59b6)

### 동작

1. 버튼 클릭 시 `adManager.isBusy` / `adManager === null` 가드
2. `adManager.showRewarded(ADMOB_REWARDED_TOWER_ID)` 호출
3. 씬 활성 상태 체크 (`this.scene.scene.isActive()`)
4. `rewarded: true` 시: `_pickAdRewardTowers()` -> `_showAdRewardModal(towers)`
5. `rewarded: false` 시: 에러 토스트 "광고를 불러올 수 없습니다" (`draw.ad.failed`, 빨간 #ff4757, 1.5초 페이드아웃)

### 타워 선택 모달

- 어두운 오버레이 (0x000000, alpha 0.6) + 컨테이너 (depth 61)
- 타워 카드 3개 (90x130px, 간격 10px) 수평 배치
- T2 카드: 금색 테두리 + 별(★) 강조
- "선택" 버튼 (60x22px, BTN_PRIMARY) 클릭 -> `_onAdTowerSelected()`
- "취소" 버튼 -> `_hideAdRewardModal()`

### 배치 처리

- T1: `onTowerSelect(type)` 호출, cost=0
- T2: `MERGE_RECIPES`에서 첫 번째 재료(base type) 추출 -> `onTowerSelect(baseType)` 호출 -> GameScene에서 `applyMergeResult(mergeData)` 적용, `totalInvested=0`
- 배치 완료 시 `_pendingAdTower = null`, `drawCount` 미증가

### 수치

| 항목 | 값 | config.js 상수 |
|---|---|---|
| T2 등장 확률 | 10% (0.10) | 코드 내 하드코딩 |
| T1 등장 확률 | 90% | 코드 내 하드코딩 |
| 카드 수 | 3개 (중복 허용) | 코드 내 하드코딩 |
| 일일 제한 | 없음 | - |
| 배치 비용 | 0G | - |
| 판매 환급 | 0G (totalInvested=0) | - |

### 예외 처리

- `adManager` null: 버튼 클릭 즉시 return
- `adManager.isBusy`: 클릭 무시
- 씬 파괴 후 콜백: `scene.scene.isActive()` 체크로 안전 종료
- 모달 중복 생성: `_showAdRewardModal` 진입 시 `_hideAdRewardModal()` 선행 호출
- T1 풀 비어있음: T2만 사용 (방어 코드)
- `destroy()` 시 `_hideAdRewardModal()` 호출로 메모리 누수 방지
...
| 보상형 (타워 뽑기) | `ca-app-pub-9149509805250873/XXXXXXXXXX` (플레이스홀더) | `ADMOB_REWARDED_TOWER_ID` |
...
- `ADMOB_REWARDED_TOWER_ID`는 플레이스홀더 값 (`XXXXXXXXXX`), 실제 배포 전 AdMob 콘솔에서 발급 필요
- 광고 보상 T2 타워 배치 시 도감(CollectionScene) 발견 기록 미처리 (별도 스펙 필요)
