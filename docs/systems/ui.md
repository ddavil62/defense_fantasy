...
**광고 진행 중 네비게이션 차단**: CAMPAIGN/ENDLESS/COLLECTION/STATISTICS 4개 버튼 핸들러에 `adManager?.isBusy` 가드 적용. "Diamond 받기" 광고 콜백에서 `this.scene.isActive('MenuScene')` 체크로 씬 전환 후 안전 보장. 음소거 토글은 씬 전환이 없으므로 가드 미적용.
