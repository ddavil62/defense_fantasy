...
- 캠페인 모드: `X/Y` 형식 (updateWave(round, total)), 검 아이콘이 웨이브를 나타내므로 "Wave:" 접두사 생략
- 엔드리스 모드: `X` 형식 (기존 동작), 아이콘 없는 폴백 시 `⚔ X` 형식
- HUD 좌표 배치 (정보 영역 x <= 250, 버튼 영역 x >= 253):
  - Wave 그룹: 아이콘 x=18, 텍스트 x=28 (폴백 x=10)
  - Diamond 그룹: 아이콘 x=103, 텍스트 x=114 (폴백 x=103)
  - HP 그룹: 아이콘 x=175, 텍스트 x=186 (폴백 x=175)
- HP <= 5 시 빨간 깜빡임
...
## TowerPanel (하단 2행 레이아웃)

패널 영역: Y=520~640 (PANEL_Y=520, PANEL_HEIGHT=120). 2행 구성.

### 1행 (Y=PANEL_Y+42): 뽑기 버튼 2개

- 뽑기 버튼 (1행 좌측 X=95, 150x44px, `slot_action_normal` 배경)
  - 상단 텍스트: "뽑기" (`draw.button`, 14px bold)
  - 하단 텍스트: 현재 비용 (예: "80G", 금색 `GOLD_TEXT_CSS`, 11px)
  - 클릭: 해금된 T1 타워 중 균등 확률 랜덤 선택 -> 배치 모드 진입
  - 골드 부족 시: alpha 0.5, 비용 텍스트 빨간색(#ff4757), 클릭 무시
  - 롱프레스(400ms): 뽑기 풀 팝업 (해금 타워 목록 + 각 확률 표시)
  - 버튼 범위: X 20~170
- 무료뽑기 버튼 (1행 우측 X=265, 150x44px, `slot_action_normal` 배경)
  - 배경: `slot_action_normal` 텍스처, 폴백 사각형(0x1a1a2e, BTN_META 테두리)
  - 상단 텍스트: "무료뽑기" (`draw.ad.button`, 14px bold 흰색) -- adFree 여부 무관하게 동일 키 사용
  - 하단 텍스트: "타워 3선택 (N/3)" (`draw.ad.sublabel`, 10px 연보라 #9b59b6)
  ...
  - 판당 3회 제한 (`AD_DRAW_LIMIT_PER_GAME`), 소진 시 비활성화
  - 버튼 범위: X 190~340
  - 두 뽑기 버튼 사이 간격: 20px (X 170~190)

### 2행 (Y=PANEL_Y+80): 배속 + 소모품 3종 + HP회복

- 배속 버튼: X=40, `icon_speed_x1`/`x2`/`x3` 아이콘(20x20), 폴백 텍스트 'x1'/'x2'/'x3'
- 소모품 버튼 3종: X=90(감속)/140(골드비)/190(번개), spacing=50px
- HP회복 버튼: X=320
- 상세는 아래 "골드 싱크 UI (하단 패널)" 섹션 참조
...
- 잠금 타워: 뽑기 풀에서 자동 제외 (해금 시 풀에 추가)
- 배치된 타워 선택 시 TowerInfoOverlay(game 모드) 오버레이 열기
- 드래그 시 합성 가능 타워 하이라이트 + 빈 셀 초록 하이라이트(이동 가능 위치) + 호버 미리보기 말풍선
- 드래그 드롭 분기: 빈 배치 가능 셀 -> 이동(무료), 합성 가능 타워 -> 합성, 그 외 -> 원위치 복원
...
- 패널 배경: NineSlice (`panel_info_overlay.png`, 300x360px 원본)로 동적 높이 대응
  - 슬라이스 상수: NS_TOP=48, NS_BOTTOM=44, NS_LEFT=24, NS_RIGHT=24
  - 모서리/테두리 장식(루비, 눈, 골드 기둥)을 유지하면서 중앙 영역만 늘림
  - 텍스처 미존재 시 Rectangle 폴백 (기존 동작 유지)
- 닫기(X) 버튼: 패널 우상단
...
- 플레이버/설명 텍스트: `wordWrap: { width: 260 }`, `align: 'center'` 적용
...
- `_isSourceWithEnhance`: game 모드 + 원래 타워(history=0) + enhanceLevel > 0일 때 true
- `enhanceOffset`: `_isSourceWithEnhance ? 16 : 0` -- 강화 레벨 텍스트로 밀린 Y를 보정

| 조건 | baseH | usedInViewH | actionH (game) | panelH |
|---|---|---|---|---|
| T1, enhance=0, 상위 조합 없음 | 240px | 0 | 90px (원래 타워) / 0 | 240~330px |
| T1, enhance>0, 상위 조합 있음 | 256px | 100px | 90px / 0 | 356~446px |
| T2+, enhance=0, 상위 조합 없음 | 316px | 0 | 90px / 0 | 316~406px |
| T2+, enhance>0, 상위 조합 있음 | 332px | 100px | 90px / 0 | 432~522px |

- game 모드에서 원래 타워(depth 0) 보기 중에만 actionH=90 추가
- `_forceClose()` 시 `_isSourceWithEnhance = false`로 리셋
...
외부 뒤로가기 -> handleBack() -> _handleBack() (공개 래퍼)
```
...
| 메서드 | 설명 |
|---|---|
| `open(towerOrEntry)` | 오버레이를 열고 타워/엔트리 정보를 렌더링한다 |
| `close()` | `_forceClose()`를 호출하여 히스토리 무시하고 즉시 닫는다 |
| `handleBack()` | `_handleBack()` 공개 래퍼. 히스토리가 있으면 이전 항목으로, 없으면 닫는다. ESC 키 핸들러 등 외부에서 뒤로가기를 트리거할 때 사용 |
| `isOpen()` | `_container !== null` 여부를 반환한다 |
| `getClosedAt()` | 마지막 닫힌 시각(ms)을 반환한다. 재오픈 방지 쿨다운 판정에 사용 |
...
드래그 시작 시(`startDrag`) 합성 가능한 모든 타워에 펄스 하이라이트를 적용하고, `onDragStart` 콜백으로 빈 배치 가능 셀에 초록색 하이라이트를 표시한다(`MapManager.showBuildableHighlights()`).
...
- **진입**: 뽑기 버튼을 400ms 이상 롱프레스
- **내용**: 현재 해금된 타워 목록과 각 확률 (`1/pool.length * 100%`) 표시
- **구조**: 컨테이너+배경+텍스트 (기존 _showDescription 팝업과 유사)
- **닫기**: 화면 아무 곳 탭
- **짧은 탭(400ms 미만)**: 뽑기 실행
...
- HUD 우측 상단 Pause 버튼 (`icon_pause` 아이콘, 폴백 '||')
...
- **부활 버튼** (판당 1회): `revived !== true`일 때 RETRY 위에 퍼플 메타 버튼 표시. 광고 시청 완료 시 현재 라운드에서 GameScene 재개 (HP = maxHP/2). 부활 가능 시 패널 높이 +56px, 버튼들 28px 하향 이동. 광고 콜백에서 `this.scene.isActive('GameOverScene')` 체크로 씬 전환 후 안전 보장
- **광고 진행 중 네비게이션 차단**: RETRY/WORLD MAP/MENU(캠페인)/MENU(엔드리스) 4개 버튼 핸들러 최상단에 `adManager?.isBusy` 가드 적용. 광고 로딩/표시 중 씬 전환 방지
- 캠페인 모드 (gameMode === 'campaign'):
  - 패널 높이 420px (부활 시 476px)
  - 부활 (BTN_META 퍼플, 판당 1회) + RETRY (BTN_PRIMARY 골드) + WORLD MAP (BTN_BACK 틸, WorldSelectScene) + MENU (BTN_DANGER 레드 160x30)
- 엔드리스 모드 (기존):
  - 패널 높이 380px (부활 시 436px)
  - 부활 (BTN_META 퍼플, 판당 1회) + RETRY (BTN_PRIMARY 골드) + MENU (BTN_DANGER 레드) 2버튼
...
- **위치**: 하단 패널 2행 우측 끝 (x=320, y=PANEL_Y+80=600)
- **크기**: 32x32px
- **라벨**: "+HP"
...
- **위치**: 하단 패널 2행, 배속 버튼(x=40)과 HP회복 버튼(x=320) 사이 (y=PANEL_Y+80=600)
- **크기**: 각 24x24px (미니 슬롯), 간격 50px
- **아이콘**: PNG 이미지 아이콘 (16x16 displaySize, depth=31). 텍스처 로드 실패 시 텍스트 폴백(S/$/Z)
- **버튼 구성**:

| 버튼 | 아이콘 텍스처 키 | 폴백 | x | 쿨다운 | 초기 비용 | 비용 증가분 |
|---|---|---|---|---|---|---|
| 슬로우 | `icon_consumable_slow` (눈꽃) | S | 90 | 30초 | 300G | +50G/회 |
| 황금비 | `icon_consumable_gold` (금화) | $ | 140 | 45초 | 400G | +75G/회 |
| 번개 | `icon_consumable_lightning` (번개) | Z | 190 | 60초 | 500G | +100G/회 |

- **쿨다운 표시**: 쿨다운 중 버튼에 잔여 시간 표시, 아이콘 alpha=0.3
- **적 없을 때**: 즉시형(슬로우, 번개)은 사용 불가, 지속형(황금비)은 사전 활성화 가능
- **연출**: 능력 사용 시 화면 플래시 (`_playAbilityFlash`)

### 소모품 툴팁

PC(마우스 호버) 또는 모바일(500ms 롱프레스) 시 버튼 위에 말풍선 형태로 능력 정보를 표시한다.

- **표시 조건**:
  - PC: `pointerover` 시 즉시 표시, `pointerout` 시 즉시 숨김
  - 모바일: `pointerdown` 후 500ms 경과 시 표시, `pointerup`/`pointerout` 시 숨김
  - 모바일 500ms 미만 탭: 툴팁 없이 능력 발동
- **위치**: 버튼 중심 x, 버튼 y - 44px (상단 방향)
- **화면 경계 보정**: 툴팁 좌우 끝이 0~360px 범위를 벗어나지 않도록 x 클램프 (halfW=65)
- **depth**: 50 (최상위)
- **구성 요소**:

| 요소 | 크기/스타일 | 내용 |
|---|---|---|
| tooltipBg | 130x52px, 0x000000, alpha 0.85 | 배경 사각형 |
| tooltipTitle | 10px bold, 흰색 | 능력 이름 (i18n `ui.{key}`) |
| tooltipDesc | 8px, #b2bec3, wordWrap 120px | 효과 설명 (i18n `consumable.{key}.desc`) |
| tooltipMeta | 7px, #ffd700 (금색) | `{cost}G | 쿨다운 {sec}초` |

- **비용 계산**: `baseCost + useCount * costIncrement` (사용할수록 증가)
- **상태 무관**: 쿨다운 중/골드 부족 상태에서도 툴팁 정상 표시
- **리소스 관리**: 버튼 수(3개)만큼만 생성, `setVisible()`로 재사용. `_destroyGoldSinkUI()`에서 tooltip 4종 + longPressTimer 정리
- **i18n 키**:

| 키 | ko | en |
|---|---|---|
| `consumable.slowAll.desc` | 모든 적의 이동속도를 50% 감소시킨다. (5초) | Reduces all enemy movement speed by 50% for 5 seconds. |
| `consumable.goldRain.desc` | 10초간 적 처치 시 골드 획득량이 2배가 된다. | Doubles gold earned from kills for 10 seconds. |
| `consumable.lightning.desc` | 모든 적에게 100의 즉발 피해를 입힌다. | Deals 100 instant damage to all enemies. |
| `ui.cooldown` | 쿨다운 {sec}초 | CD {sec}s |

### 레이아웃 참고

```
하단 패널 (y=520~640, 2행 레이아웃)
┌──────────────────────────────────────────┐
│  [  뽑기 80G  ]      [  무료뽑기  ]       │ 1행 y=562 (PANEL_Y+42)
│  (X=95,150x44)       (X=265,150x44)     │
│ [Speed][S][$][Z]              [+HP]      │ 2행 y=600 (PANEL_Y+80)
│ (X=40)(90)(140)(190)          (X=320)    │
└──────────────────────────────────────────┘
```
...
- 하단 좌측: "2배 골드"(70x18px, cardLeft+38) + "보상 2배"(70x18px, cardLeft+114) 광고 버튼
  - 기본: 퍼플(BTN_META) 배경 + 흰색 텍스트(9px)
  - 활성화: 녹색(0x00b894) 배경 + 금색 텍스트 + "ON" 접미사
  - 한도 소진: 회색(BTN_SELL) 배경 + 회색 텍스트, non-interactive
- 하단 우측: START 버튼 (BTN_PRIMARY, 13px bold)
- **광고 진행 중 네비게이션 차단**: BACK/START 2개 버튼 핸들러에 `adManager?.isBusy` 가드 적용. "2배 골드"/"보상 2배" 광고 콜백에서 `this.scene.isActive('LevelSelectScene')` 체크로 씬 전환 후 안전 보장
...
| Y | 버튼 | 크기 | 이미지 키 | 동작 |
|---|---|---|---|---|
| 302 | Diamond 받기 | 소형 140x26 | `btn_small_meta` / `btn_small_disabled` | 보상형 광고 시청 -> Diamond +3 지급 (일일 5회, 잔여 표시) |
...
| 596 | 음소거 토글 | 소형 80x26 | `btn_small_back` / `btn_small_disabled` | 음소거 ON/OFF, X=centerX-48 |
| 596 | 언어 토글 | 소형 80x26 | `btn_small_back` | ko/en 순환 전환, X=centerX+48 |

- 음소거 버튼(centerX-48)과 언어 버튼(centerX+48)이 16px 간격으로 나란히 배치
- 언어 버튼 레이블: ko='한국어', en='English' (`localeLabels` 객체, 미등록 로케일은 코드 자체를 폴백 표시)
- 언어 전환 시 `setLocale(next)` + `this.scene.restart()`로 씬 재시작하여 전체 텍스트 갱신
- 언어 설정은 localStorage(`fantasy-td-lang` 키)에 저장되어 앱 재시작 시 자동 복원
- 언어 버튼은 SoundManager 존재 여부와 무관하게 항상 표시 (음소거 버튼은 `if (sm)` 블록 내)

모든 버튼은 `_createImageButton()`으로 생성. 이미지 텍스처 존재 시 Image, 미존재 시 Rectangle 폴백.

**광고 진행 중 네비게이션 차단**: CAMPAIGN/ENDLESS/COLLECTION/STATISTICS 4개 버튼 핸들러에 `adManager?.isBusy` 가드 적용. "Diamond 받기" 광고 콜백에서 `this.scene.isActive('MenuScene')` 체크로 씬 전환 후 안전 보장. 음소거 토글/언어 토글은 씬 전환이 없으므로 가드 미적용.
...
- `js/scenes/BootScene.js` preload()에서 56장 일괄 로드
...
| 유형 | 크기 | 키 | 용도 |
|---|---|---|---|
| 타워 슬롯 | 40x40px | `tower_slot_normal/selected/locked` | (레거시, 현재 미사용) |
...
| `panel_info_overlay` | 300x360px | TowerInfoOverlay 패널 (NineSlice 소스, 동적 높이) |
...
| `icon_consumable_slow` | 24x24px | 소모품 슬로우 (눈꽃) |
| `icon_consumable_gold` | 24x24px | 소모품 황금비 (금화) |
| `icon_consumable_lightning` | 24x24px | 소모품 번개 (번개) |
| `icon_sell` | 64x64px (표시 20x20) | 판매 텍스트 'S' |
| `icon_speed_x1` | 64x64px (표시 20x20) | 배속 텍스트 'x1' |
| `icon_speed_x2` | 64x64px (표시 20x20) | 배속 텍스트 'x2' |
| `icon_speed_x3` | 64x64px (표시 20x20) | 배속 텍스트 'x3' |
| `icon_pause` | 64x64px (표시 18x18) | 일시정지 텍스트 '||' |
| `icon_sound_on` | 64x64px (표시 16x16) | 사운드 켜짐 텍스트 '♪' |
| `icon_sound_off` | 64x64px (표시 16x16) | 음소거 텍스트 'M' |
...
- 장식: 3장 (미사용)
- 아이콘: 16장 (HUD 6장 + 소모품 3장 + 게임 플레이 UI 버튼 7장)
- **합계: 56장**
...
── 오버레이 우선 처리 (switch 분기 전) ──
MergeCodexScene + towerInfoOverlay.isOpen() -> handleBack() + return
GameScene + towerPanel.isOverlayOpen() -> towerPanel.towerInfoOverlay.handleBack() + return
GameScene + isPaused && pauseOverlay -> _resumeGame() + return

씬별 switch 분기 (오버레이 미해당 시)
```

### 오버레이 우선 처리

`switch (key)` 분기 전에 오버레이 열림 상태를 먼저 확인한다. TowerInfoOverlay(depth 100)가 pauseOverlay(depth 50)보다 우선한다.

| 씬 | 조건 | 동작 |
|---|---|---|
| MergeCodexScene | `towerInfoOverlay.isOpen()` | `towerInfoOverlay.handleBack()` (히스토리 pop 또는 닫기) |
| GameScene | `towerPanel.isOverlayOpen()` | `towerPanel.towerInfoOverlay.handleBack()` (히스토리 pop 또는 닫기) |
| GameScene | `isPaused && pauseOverlay` | `_resumeGame()` (게임 재개) |

오버레이가 열려 있지 않으면 아래 씬별 매핑으로 진행한다.
...
`js/i18n.js`에서 모든 UI 텍스트를 한국어(ko)와 영어(en)로 관리한다. `t(key)` 함수로 현재 로케일에 맞는 문자열을 조회한다.

### 언어 전환

- **MenuScene 언어 토글 버튼**: 음소거 버튼 우측에 배치, 탭 시 ko -> en -> ko 순환 전환
- **localStorage 저장**: `fantasy-td-lang` 키에 현재 로케일 저장, 앱 재시작 시 i18n.js 모듈 초기화에서 자동 복원
- **지원 로케일**: `SUPPORTED_LOCALES = ['ko', 'en']` (i18n.js export). 추후 언어 추가 시 배열에 항목 추가 + STRINGS 키 추가로 확장 가능
- **기본값**: localStorage 미설정/접근 실패/잘못된 값 시 'ko' 유지
- **전환 방식**: `setLocale(next)` + `this.scene.restart()` (개별 텍스트 동적 갱신 아님)

### 커버리지

- 총 키 수: **467개** (ko 467 = en 467, 완전 대칭)
- 중복 키: 0건
- fallback 체인: `STRINGS[_locale]?.[key] ?? STRINGS['ko'][key] ?? key`
- 플레이스홀더 패턴: `{variable}` 형식, `t(key).replace('{var}', value)` 패턴으로 치환

### t() 전환 완료 파일

| 파일 | 전환 수 | 주요 키 |
|---|---|---|
| `StatsScene.js` | ~22 | `stats.*`, `stats.recentInfo`, `stats.recentBoss` |
| `GameOverScene.js` | ~15 | `gameover.*` |
| `GameScene.js` | ~12 | `pause.*`, `pause.sfx`, `pause.bgm` |
| `CollectionScene.js` | ~8 | `collection.tier`, `collection.tierLabel`, `collection.effectBaseHp`, `collection.effectGoldBoost`, `collection.effectSpeedBoost`, `collection.currentStatus`, `collection.upgrade` |
| `TowerInfoOverlay.js` | ~4 | `tower.sell`, `tower.statLine`, `ui.backLabel` |
| `EndlessMapSelectScene.js` | ~3 | `endless.waveInfinity`, `endless.waveCount` |
| `MenuScene.js` | ~3 | `menu.*` |
| `MergeCodexScene.js` | ~1 | `codex.*` |
| `MapClearScene.js` | ~1 | `mapclear.*` |

### 알려진 잔존 하드코딩 (스코프 외)

| 심각도 | 파일 | 내용 | 사유 |
|---|---|---|---|
| LOW | `LevelSelectScene.js` | `` `Wave: ${totalW}` `` | 별도 과제 |
| LOW | `CollectionScene.js` | `` `${displayName} Tower` `` | 잠금 해제 팝업, 별도 과제 |
| INFO | `MenuScene.js` | `'Fantasy\nTower Defense'` | 브랜드명, 의도적 |
| INFO | `TowerInfoOverlay.js` | Single/Splash/AoE/Chain/Beam/DoT | 범용 게임 용어, 의도적 |
| INFO | `GameScene.js` | 음소거 아이콘 M/음표 | 기호, 의도적 |
