...
- **크기**: 각 24x24px (미니 슬롯), 간격 26px
- **아이콘**: PNG 이미지 아이콘 (16x16 displaySize, depth=31). 텍스처 로드 실패 시 텍스트 폴백(S/$/Z)
- **버튼 구성**:

| 버튼 | 아이콘 텍스처 키 | 폴백 | x | 쿨다운 | 초기 비용 | 비용 증가분 |
|---|---|---|---|---|---|---|
| 슬로우 | `icon_consumable_slow` (눈꽃) | S | 244 | 30초 | 300G | +50G/회 |
| 황금비 | `icon_consumable_gold` (금화) | $ | 270 | 45초 | 400G | +75G/회 |
| 번개 | `icon_consumable_lightning` (번개) | Z | 296 | 60초 | 500G | +100G/회 |
